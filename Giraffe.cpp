...
void Giraffe::Move(Stage& stage, const int frameNumber, std::array<Giraffe*, 4> giraffes, std::vector<Line>& lines)
{
	...
	if (!(State & STATE_LEDGEHOG)) {
		if (stage.Intersects(Position, StageCollider, State & (STATE_CROUCH | STATE_FASTFALL), State & STATE_JUMPING, Velocity.y > -0.00001, State & STATE_HITSTUN, landed, bounced, Facing, offset, Velocity, hogging, LedgeID)) {
			StageIntersection(stage, frameNumber, giraffes, offset, hogging, bounced, landed);
		}
...
void Giraffe::ParseInputs(const int inputs, const int frameNumber, Stage& stage)
{
	if (!(State & (STATE_JUMPSQUAT | STATE_JUMPLAND | STATE_DROPSHIELD | STATE_SHIELDSTUN | STATE_KNOCKDOWNLAG | STATE_ROLLING | STATE_GRABBED | STATE_THROW))) {
		ParseWalk(inputs, frameNumber, stage);
	}
	if (inputs & INPUT_WEAK && !(State & (STATE_WEAK | STATE_HEAVY | STATE_SHIELDSTUN | STATE_DROPSHIELD | STATE_JUMPSQUAT | STATE_JUMPLAND | STATE_HITSTUN | STATE_WAVEDASH | STATE_KNOCKDOWNLAG | STATE_TECHING | STATE_ROLLING | STATE_GRABBING | STATE_GRABBED | STATE_THROW))) {
		ParseWeak(inputs, frameNumber, stage);
	}
	if (inputs & INPUT_HEAVY && !(State & (STATE_WEAK | STATE_HEAVY | STATE_SHIELDING | STATE_DROPSHIELD | STATE_JUMPSQUAT | STATE_JUMPLAND | STATE_HITSTUN | STATE_WAVEDASH | STATE_LEDGEHOG | STATE_KNOCKDOWN | STATE_KNOCKDOWNLAG | STATE_TECHING | STATE_ROLLING | STATE_GRABBING | STATE_GRABBED | STATE_THROW))) {
		ParseHeavy(inputs, frameNumber, stage);
	}
	if (inputs & INPUT_JUMP && !(State & (STATE_WEAK | STATE_HEAVY | STATE_JUMPSQUAT | STATE_JUMPLAND | STATE_HITSTUN | STATE_SHIELDSTUN | STATE_WAVEDASH | STATE_KNOCKDOWNLAG | STATE_TECHING | STATE_ROLLING | STATE_GRABBING | STATE_GRABBED | STATE_THROW))) {
		ParseJump(inputs, frameNumber, stage);
	}
...
void Giraffe::ParseWeak(const int inputs, const int frameNumber, Stage& stage)
{
	State |= STATE_WEAK;
	State &= ~STATE_CROUCH;
...
void Giraffe::ParseHeavy(const int inputs, const int frameNumber, Stage& stage)
{
	State &= ~STATE_CROUCH;
	State |= STATE_HEAVY;
	if ((inputs & INPUT_RIGHT && Facing.x == 1) || (inputs & INPUT_LEFT && Facing.x == -1)) {
...
void Giraffe::ParseJump(const int inputs, const int frameNumber, Stage& stage)
{
	if (State & STATE_LEDGEHOG) {
		State &= ~STATE_LEDGEHOG;
		stage.Ledges[LedgeID].Hogged = false;
...
void Giraffe::ParseShield(const int inputs, const int frameNumber, Stage& stage)
{
...
void Giraffe::RecieveHits(Stage& stage, const int frameNumber)
{
	if (incomingGrab) {
...
void Giraffe::StageIntersection(Stage& stage, const int frameNumber, std::array<Giraffe*, GGPO_MAX_PLAYERS> giraffes, Vector2 offset, bool hogging, bool bounced, bool landed)
{
	Position += offset;
	if (hogging) {
		Hogging(stage, frameNumber, giraffes);
	}
	else if (bounced) {
		Bouncing(stage, frameNumber, giraffes);
	}
	else if (landed && (State & STATE_JUMPING)) {
		Landing(stage, frameNumber, giraffes);
	}
}

void Giraffe::Hogging(Stage& stage, const int frameNumber, std::array<Giraffe*, GGPO_MAX_PLAYERS> giraffes)
{
...
void Giraffe::Bouncing(Stage& stage, const int frameNumber, std::array<Giraffe*, GGPO_MAX_PLAYERS> giraffes)
{
...
void Giraffe::Landing(Stage& stage, const int frameNumber, std::array<Giraffe*, GGPO_MAX_PLAYERS> giraffes)
{
...
void Giraffe::Die(Stage& stage, const int frameNumber, std::vector<Line>& lines)
{
